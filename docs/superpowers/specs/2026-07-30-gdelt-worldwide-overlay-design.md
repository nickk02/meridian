...
3. **Render layer in `MapView.tsx`**. A plain MapLibre `circle` layer, the same primitive already used for `objects`, `ais`, and `objects-glow`. GDELT points are 2D surface points whose radius should scale with article count, exactly the shape `DOT_RADIUS`/`GLOW_RADIUS` already handle for severity via `["interpolate", ["linear"], ["get", "severity"], ...]` expressions; the same pattern applies here keyed on a `count` property instead. deck.gl is not needed: it is reserved in this codebase for the satellite orbital-altitude case specifically (a true 3D need MapLibre cannot express), not for 2D density points. (Corrected 2026-07-30 after reading `MapView.tsx` in full; the original draft of this spec proposed `ScatterplotLayer`, which would have introduced deck.gl for a case that does not need it.)
